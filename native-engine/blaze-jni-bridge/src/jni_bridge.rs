...
macro_rules! jni_get_direct_buffer {
    ($value:expr) => {{
        ...
        println!("pos: {}, remaining: {}", pos, remaining);
        $crate::jni_bridge::THREAD_JNIENV.with(|env| {
            println!("env.get_direct_buffer_address($value.into()): {:?}", env.get_direct_buffer_address($value.into()));
...
impl<'a> JniUtil<'a> {
    pub const SIG_TYPE: &'static str = "org/apache/flink/table/planner/plan/nodes/exec/glue/jni/JniUtil";
...
impl<'a> BlazeConf<'_> {
    pub const SIG_TYPE: &'static str = "org/apache/flink/table/planner/plan/nodes/exec/glue/jni/BlazeConf";
